...
- The datagram socket is an Expo module in [packages/entangle-udp](packages/entangle-udp) (Swift + Kotlin), consumed as `"entangle-udp": "workspace:*"`. It deliberately does **not** live in `apps/mobile/modules`: a local module there is found through autolinking's `nativeModulesDir`, which resolved differently on EAS Build than locally and silently shipped an app with no module in it. As a workspace dependency it is found through the same node_modules search path as every other Expo module. `send` is a synchronous `Function`, not an `AsyncFunction`: it runs up to 120 times a second and a promise per frame would cost more than the frame.
