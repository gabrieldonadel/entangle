...
- Local Expo module [modules/entangle-udp](apps/mobile/modules/entangle-udp) (Swift + Kotlin) owns the datagram socket. `send` is a synchronous `Function`, not an `AsyncFunction`: it runs up to 120 times a second and a promise per frame would cost more than the frame.
...
- Pointer frames take the **datagram path** when the Mac offers one: `welcome.udp` hands over a port and a session token, the phone wraps each frame as `{v, tk, m}` and sends it over UDP ([apps/mobile/modules/entangle-udp](apps/mobile/modules/entangle-udp), [DatagramServer.swift](apps/desktop/modules/entangle-server/ios/Server/DatagramServer.swift)). Everything else — pairing, dock, audio, state, ping — stays on the WebSocket.
