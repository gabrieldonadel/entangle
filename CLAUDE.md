...
- The native module lives at [apps/desktop/modules/entangle-server](apps/desktop/modules/entangle-server) — local Expo module, Apple-only ([expo-module.config.json](apps/desktop/modules/entangle-server/expo-module.config.json)).
  ...
  - [System/](apps/desktop/modules/entangle-server/ios/System) — `CursorController`, `ScrollController`, `KeyController`, `GestureController`, `DockEnumerator` (CGEvent / Accessibility APIs), `PointerHighlight` (click-through overlay ring shown while a phone is connected).
